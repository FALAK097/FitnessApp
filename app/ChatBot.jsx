...
const ChatBot = () => {
    ...
    const getResponse = async () => {
        ...
        try {
            ...
            Alert.alert("Error", "Failed to get response from language model. Please try again later.");
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <View style={styles.chatContainer}>
                <ScrollView ref={scrollViewRef} style={styles.scrollView} contentContainerStyle={[styles.contentContainer, { backgroundColor: theme.mainBackgroundColor }]}>
                    {messages.map((message, index) => (
                        <View key={index} style={message.author === 'You' ? styles.userMessageContainer : styles.botMessageContainer}>
                            <Text style={message.author === 'You' ? styles.userMessage : styles.botMessage}>
                                {message.author === 'You' ? 'You: ' : 'FitHub GPT: '}{message.content}
                            </Text>
                        </View>
                    ))}
                    {isLoading && (
